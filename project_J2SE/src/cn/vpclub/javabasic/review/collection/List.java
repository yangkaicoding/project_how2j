...
public class List {
     ...
     *      数组存放的数据类型都是基本数据类型(四类八种) 集合存放的数据类型还可以是引用数据类型(String, Integer, 自定义数据类型)；集合中对于基本数据类型会转换为引用数据类型再进行存储
     */

    /**
     * 集合包含的内容、集合的框架：
     * 接口类：Collection,Map,Set,List等(其中Set和List继承了Collection)
    ...
    /**
     * List：
     * List集合是一个有序的、可重复的集合，集合每一个元素都有对应的顺序索引
     * List集合中允许添加重复的元素，是因为可以通过索引来访问指定位置的元素
     * List集合中默认按照元素的添加顺序来增加元素的索引
     */

    /**
     * ArrayList：
     * ArrayList是基于数组实现的List类，实现所有可选列表操作，且允许所有的元素包括null
     * ArrayList arrayList = new ArrayList()       初始化容量为10的列表集合
     * ArrayList<E> arrayList = new ArrayList<E>() 数据类型为E,初始容量为10
     *
     * 主要方法：
     * boolean add(E e) 将指定的元素追加到次列表的末尾
    ...
    /**
     * LinkedListClass：
     * LinkedList指的是链表类的数据结构，链表中的元素可以任意的增加和删除，但是查询效率不如列表，链表将对象存放在独立的空间中，而且每个空间都保存了下一个链接的索引
     *
     * 主要方法：
     ...
     * E peekLast()  检索但不删除此列表最后一个元素，如果此列表为空，则返回null
     ...
     *      JDK1.5之前使用for循环对集合、数组、迭代器进行遍历，需要创建索引变量、条件表达式，会导致代码臃肿，增加代码编写时的出错几率
     *
     * 语法：for(Type e : collections){
     *          //对变量e的使用语句
     *  }
     * 参数：e其类型Type是集合或数组中元素值得类型，该参数是集合或数组collections中的一个元素
     *      collection是要遍历的集合或数组，也可以是迭代器，在循环中使用参数e,该参数是foreach从集合或数组以及迭代器中取得的元素值，元素值是从头到尾遍历的
     */
