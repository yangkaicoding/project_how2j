...
public class AccuratelyFloat {
     ...
     * 方法：加法 public BigDecimal add (BigDecimal augend)          参数说明：augend:与当前对象执行加法的操作数
     ...
     *
     * 技巧：这里创建BigDecimal类的实例时，在构造方法中一定要使用数字字符串作为参数；如果直接使用浮点数或该类型的变量作为参数
     *      那么构造方法接收的是经过二进制存储的浮点数，那样就会是不精确的浮点数
     */
